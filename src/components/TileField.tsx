  ...
  return new THREE.ShaderMaterial({
    ...
    vertexShader: /* glsl */ `
      ...
      void main() {
        ...
        // المكعّب المستهلَك يخرج من هنا، لا من الـfragment shader.
        //
        // كان الرفض يحدث بـdiscard بعد التنقيط (rasterization): تُرسم شظايا كل مكعّب منتهٍ ثم
        // تُرمى واحدة واحدة. وأسوأ من ثمنها المباشر أن مجرّد وجود discard في الـshader يُعطّل
        // اختبار العمق المبكّر (early-Z) للمشهد كله — لا تعود البطاقة تعرف مسبقاً أي شظية
        // محجوبة، فتُظلَّل شظايا مخفيّة خلف مكعّبات أمامها. والحقل مائل، أي أن التداخل هو
        // القاعدة لا الاستثناء.
        //
        // وهذا يخصّ الأحزمة القصيرة أكثر من الجميع: حزام التواصل يصرف 64% من ارتفاعه على
        // التلاشي (BAND_FADE 0.36 + 0.28)، فأغلب مكعّباته منتهية أو شبه منتهية — أي أن أغلب ما
        // كان يُنقَّط كان يُرمى.
        //
        // دفع الرأس خارج مكعّب القصّ يُسقط المثلّث كاملاً قبل التنقيط. وk تُحسب من مركز الخلية
        // لا من الرأس، فرؤوس المكعّب الثمانية تخرج معاً — لا نصف مكعّب ولا وجه مبتور. النتيجة
        // على الشاشة هي هي، بلا شظية واحدة مهدورة.
        if (k < 0.02) {
    ...
    fragmentShader: /* glsl */ `
      ...
      void main() {
        // المكعبات المستهلَكة تغادر بدلاً من أن ترقد مسطّحة — لكن مغادرتها صارت في الـvertex
        // shader (انظر الملاحظة هناك)، فلا يصل إلى هنا أصلاً ما كان يُرمى. لا discard في هذا
        // الملف، وهذا شرط بقاء early-Z عاملاً لبقيّة الحقل.
...
const Field: React.FC<{ reduced: boolean; tones: FieldTones; fade: FieldFade }> = ({
  ...
  /* صفوف وأعمدة هامشية، والإمالة هي ما تدفع ثمنها. إمالة الشبكة تُقصِّر ارتفاعها المُسقَط بمقدار
     ...
     ويترك رملاً عارياً على جانبين. القسمة على cos أعلاه هي التي تعوّض ذلك؛ الهامش هنا فائض فوقها،
     لتغطية ارتفاع المكعب البارز عند الأعلى وتغيير الحجم أثناء السحب.

     ثلاثة في كل اتجاه، لا خمسة. الرقم كان ثابتاً بغضّ النظر عن حجم الحقل، وهذا يمرّ دون أن
     يُلاحَظ على الهيرو (شاشة كاملة: عشرات الصفوف، فالهامش نسبته صغيرة) ويصبح الأغلبية على حزام
     ارتفاعه clamp(8rem, 15vw, 15rem): حزام فيه أربعة صفوف حقيقية كان يحمل أربعة عشر — أي أن
     ثلثي مكعّباته لم تكن على الشاشة أصلاً، وتُحسب وتُرسم في كل إطار. وهذا يقع مرّتين في أسفل
     الصفحة، لأن حزام التواصل وحزام الفوتر متجاوران وكلاهما يعمل في نفس اللحظة.

     ±3 خليّة تبقى أوسع بكثير من أي بروز حقيقي: أطول مكعّب 0.86 خليّة، ويُسقط ارتفاعه رأسياً
     بـsin(TILT_X) أي أقل من نصف خليّة. */
  const cols = Math.ceil(size.width / cellPx / Math.cos(TILT_Y)) + 6;
  const rows = Math.ceil(size.height / cellPx / Math.cos(TILT_X)) + 6;
  const count = cols * rows;

  const geometry = useMemo(() => new THREE.BoxGeometry(1, 1, 1), []);

  /* المادة تُبنى على قيم الألوان والأحزمة، لا على هويّة الكائن الذي يحملها.
   *
   * كانت الاعتماديّة هي `tones` و`fade` أنفسهما. مستهلك يمرّر ثابت وحدة (حزام التواصل:
   * SIGNAL_TONES) بخير؛ ومستهلك يبني كائنه في نداء العرض — وهذا ما يفعله الفوتر حرفياً:
   * `connectionTones(...)` تُنشئ كائناً جديداً في كل عرض — كان يحصل على ShaderMaterial جديدة كل
   * مرّة، أي ترجمة GLSL جديدة على الخيط الرئيسي لنفس الـshader بالضبط. وترجمة shader ليست عملاً
   * رخيصاً يُقاس بالميكروثانية: هي توقّف مرئي، ويقع بينما تمرّ الصفحة تحت العين.
   *
   * المفتاح النصّي يجعل هذا مستحيلاً لكل مستهلك، حاضر أو قادم، بدل أن يُطلب من كل موضع نداء أن
   * يتذكّر التغليف بـuseMemo — والنسيان هناك لا يُنتج خطأ، بل بطئاً صامتاً. */
  const toneKey = `${tones.trough}|${tones.crest}|${tones.foam}|${tones.ground}|${tones.intoLo}|${tones.intoHi}`;
  const fadeKey = `${fade.lo}|${fade.hi}`;
  // eslint-disable-next-line react-hooks/exhaustive-deps
  const material = useMemo(() => makeFieldMaterial(cell, tones, fade), [cell, toneKey, fadeKey]);
  ...
  useFrame((_, delta) => {
    ...
    material.uniforms.uTime.value = clock.current;
    // لا invalidate() هنا. هذه الدالة لا تعمل إلا والحلقة على 'always' (على 'never' لا تعمل،
    // وعلى 'demand' — حالة تقليل الحركة — تعود من السطر الأول)، و'always' ترسم كل إطار بلا
    // طلب. كان النداء يمرّ على مخزن الحالة في كل إطار من كل حزام مقابل لا شيء.
  });
 ...
 * المكعب.
 */
const TileFieldHost: React.FC<TileFieldProps> = ({ tones = HERO_TONES, fade = HERO_FADE }) => {
...
/* memo، لأن مستهلكيه أقسام بحالة داخلية.
 *
 * قسم التواصل يحمل نموذجاً بحقول متحكَّم بها: كل ضغطة مفتاح فيه تُعيد عرض القسم، وكانت تُعيد
 * عرض هذا المضيف معه — و<Canvas> معه، ومعه مطابقة شجرة R3F كاملة — بينما لا شيء يخصّ الحقل
 * تغيّر: خمسة ألوان ورقمان، وكلها ثوابت وحدة.
 *
 * المقارنة السطحية تكفي بعد المفتاح النصّي أعلاه: مستهلك يمرّر كائناً جديداً بنفس القيم يمرّ من
 * memo ثم لا يعيد بناء المادة على أي حال. */
export const TileField = React.memo(TileFieldHost);
