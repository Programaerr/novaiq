...
function makeWaterMaterial(): THREE.ShaderMaterial {
  return new THREE.ShaderMaterial({
    ...
    fragmentShader: /* glsl */ `
      ...
      // نفس الموجة مرتين، الثانية أصغر بحوالي ثلاث مرات، وهذا ما يفرّق بين ماء وتدرّج.
      //
      // الموجة وحدها كافية على حقل المكعبات لأن هناك ما يعطيها نسيجاً: التقطيع إلى خلايا، وصور
      // ظلّ المكعبات، وثلاثة أوجه لكل واحد. اللوح أملس وليس فيه أي من ذلك، فطول موجة يقطع 523
      // بكسل مرة ونصف تقريباً يُقرأ عليه شريطاً واحداً مائلاً — أي تدرّجاً ساكناً، وهو بالضبط ما
      // أظهرته أول لقطة.
      //
      // الطبقة الثانية لا تغيّر الموجة الأولى، تضيف فوقها فقط: نفس دوال الوقت على طول موجة أقصر،
      // فتبدو أسرع بنفس النسبة — وهكذا يتحرك الماء الحقيقي، تموّج صغير يركب على انتفاخ كبير.
      // والإزاحة كي لا تصطف قمم الطبقتين على نفس الخط وتصنعا نمطاً واضح التكرار.
      float ripple(vec2 p) {
        return swell(p)
             + swell(p * 2.9 + vec2(7.1, 3.3)) * 0.34
             + swell(p * 6.3 + vec2(2.7, 9.1)) * 0.13;
      }

      void main() {
        vec2 p = vUv * uSize / ${WAVE_PX.toFixed(1)};

        // الميلة تُؤخذ بالفروق لا بالاشتقاق التحليلي: ست جيوب مشتقّة يدوياً هي ستة أماكن إضافية
        // يمكن أن تفترق فيها هذه الموجة عن موجة الحقل، والفرق هنا يكلّف تقييمين.
        float e = 0.35;
        float h = ripple(p);
        float dx = ripple(p + vec2(e, 0.0)) - h;
        float dy = ripple(p + vec2(0.0, e)) - h;
        ...
        float glint = smoothstep(0.5, 1.0, lit);
        glint *= glint;
        col += uGlint * (glint * glint * uGlintAmt);

        gl_FragColor = vec4(col, 1.0);

        // ولا بد من هذا السطر هنا تحديداً.
        //
        // THREE.Color تحوّل كل uniform من sRGB إلى ضوء خطّي عند الدخول، وShaderMaterial خام لا
        // يعيد التحويل عند الخروج ما لم يُطلب منها. أول رسم لهذه اللوحة خرج شبه أسود: 0.020 خطّية
        // كُتبت كما هي في مخزن sRGB فقُرئت 5/255 بدل 39/255.
        //
        // حقول المكعبات لا تُظهر هذا العيب لأن ألوانها كلها قريبة من الأبيض، وهناك الخطأ صغير
        // (0.939 تُقرأ 239 بدل 247). كل الخطأ يقع في الطرف الداكن، وهذا اللوح داكن.
        //
        // linearToOutputTexel (وهذا ما يوسّعه هذا الـinclude) تقرأ فضاء خرج الرسّام بدلاً من
        // كتابة معادلة sRGB هنا بخط اليد — فلو تغيّر ذلك الإعداد يوماً تتبعه هذه اللوحة معه.
        //
        // (ولا علامة backtick واحدة في السطرين أعلاه، رغم أنهما يسمّيان دالة: هذا نص قالب، وعلامة
        //  واحدة تغلقه في منتصف الـshader. حصل ذلك فعلاً هنا قبل سطرين من كتابة هذا التحذير.)
    ...
    <div ref={hostRef} aria-hidden="true" className="absolute inset-0">
      {built && (
        <Canvas
          ...
          // نفس سقف حقل المكعبات: 1 على مؤشر خشن (الهاتف)، و1.25 على غيره.
          //
          // كان 1 ثابتاً هنا بحجّة أن هذا سطح متدرّج بلا حافة حادة. صحّ ذلك حتى ظهر البريق: هو
          // أصغر تفصيل في اللوحة وأشدّه تبايناً، وهو بالضبط ما يبهت على شاشة كثيفة عند dpr 1.
          // واللوح صغير (523×528 هنا)، فالفرق بضع مئات الآلاف من البكسلات لا الملايين.
          dpr={[1, COARSE ? 1 : 1.25]}
