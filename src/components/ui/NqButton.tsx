...
export const NqButton = React.forwardRef<HTMLButtonElement, NqButtonProps>(function NqButton(
  {
    ...
    type = 'button',
    tileSurface,
    /* شبكة المربّعات تحت المؤشّر — مُشغَّلة افتراضياً من جديد.
     *
     * كانت مطفأة لأن النسخة السابقة منها كانت مشهد WebGL يُنشأ **داخل معالج الضغطة نفسه**:
     * WebGLRenderer وترجمة shader، متزامنَين قبل أن يعود المتصفح للرسم — وهو ما ظهر في
     * الكونسول كـ"[Violation] 'click' handler took Nms" عند كل نقرة، يتبعه
     * "Context Lost" عند التفكيك.
     *
     * الأثر الآن CSS خالص (انظر `.nq-tiles`): لا سياق، ولا سقف عدد، ولا مؤقّتات — فلا سبب
     * لإطفائه، ويعمل على كل زرّ في آنٍ واحد بدل واحد في كل الموقع. */
    tiles = true,
    disabled,
    onPointerEnter,
    onPointerMove,
