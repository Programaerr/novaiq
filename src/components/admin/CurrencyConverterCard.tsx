    ...
    <div className="p-5 rounded-2xl bg-paper border border-ink/10 space-y-3">
      <div className="flex items-center justify-between gap-2">
        ...
        <button
          ...
          className="p-2 rounded-lg bg-white/70 border border-ink/10 text-ink/60 hover:text-ink disabled:opacity-50 cursor-pointer transition-colors"
      ...
      <div className="flex items-center gap-2">
        <div className="flex-1">
          <label className="block text-[11px] font-semibold text-ink/50 mb-1.5">{fromLabel}</label>
          <PriceInput
            value={amount}
            onChange={setAmount}
            placeholder="0"
            className="w-full px-3 py-2.5 rounded-xl bg-white/70 border border-ink/10 text-ink text-sm font-mono"
          />
        </div>
        <button
          onClick={() => setDirection((d) => (d === 'iqd_to_usd' ? 'usd_to_iqd' : 'iqd_to_usd'))}
          title={isAr ? 'عكس الاتجاه' : 'Swap direction'}
          className="mt-5 shrink-0 p-2.5 rounded-xl bg-white/70 border border-ink/10 text-ink/60 hover:text-ink cursor-pointer transition-colors"
        >
      ...
      <div className="pt-1 border-t border-white/5 space-y-2">
        <label className="flex items-center gap-2 text-[11px] font-semibold text-ink/60 cursor-pointer select-none">
          <input
            type="checkbox"
          ...
          />
          <SlidersHorizontal className="w-3.5 h-3.5 text-ink/50" />
      ...
      {!useCustomRate && updatedAt && (
        <p className="text-[10px] text-ink/45" dir="ltr">
