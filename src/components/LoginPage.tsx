...
export const LoginPage: React.FC<LoginPageProps> = ({ language, onContinueAsGuest }) => {
  ...
  const handleGoogleSignIn = async () => {
    ...
    /* لا مراقب إلغاء بعد اليوم، ولا حاجة إليه.
     *
     * كان هنا منطق كامل يراقب عودة التركيز إلى النافذة ليكتشف "أغلق نافذة Google دون دخول" —
     * وُجد لأن Firebase يستفتي `popup.closed` وترويسة COOP في صفحة Google تقطع ذلك الاستفتاء،
     * فيبقى الوعد معلّقاً ويدور الزرّ إلى الأبد.
     *
     * وSupabase لا يفتح نافذة أصلاً: الصفحة كلها تغادر إلى Google وتعود. فلا نافذة تُغلق ولا
     * وعد يعلّق — من تراجع عند Google يعود إلى هذه الصفحة نفسها بلا جلسة، والزرّ في حالته
     * الأولى لأن المكوّن رُكِّب من جديد. الحالة الوحيدة التي تصل إلى `catch` هنا هي فشل بدء
     * التوجيه نفسه (شبكة، أو رابط عودة غير مسجَّل في إعدادات Supabase). */
    try {
      const { error } = await loginWithGoogle();
      if (error) throw error;
      // نجاح البدء يعني أن المتصفح يغادر الآن — لا شيء بعده يعمل، ولا يُطفأ الدوران عمداً.
    } catch (err) {
