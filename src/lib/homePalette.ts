 ...
 * يدعم ذلك: الأبيض على `#FF8E3D` نسبته 2.29:1، وهذا أقل من حد 3:1 حتى للنص الكبير. Obsidian على
 * نفس البرتقالي نسبته 8.67:1. كل زوج أدناه يضع نصاً على البرتقالي أو Ember يستخدم العضو الداكن من
 ...
 * منطقة الأصفر الذهبي بدلاً من ذلك، بعيداً بما يكفي عن `#FF8E3D` على عجلة الألوان بحيث لا يُخلط
 * بينهما أبداً.
 */
 ...
 */
export const ORANGE = '#FF8E3D';
/** برتقالي، مُعتَّم باتجاه Obsidian حتى يتجاوز 4.5:1 على الأبيض الدافئ — لرابط أو نص صغير يجب أن
 *  يكون برتقالياً لكنه يجلس على أرضية فاتحة، حيث البرتقالي الصافي نسبته 2.13:1 فاشلة.
 *
 *  هذا الرمز لم يتحرك حين فُتّح ORANGE، ولا يقدر: هو أصلاً عند سقفه. بالقياس، `#BA4F04`
 *  نسبته 4.65:1 على الأبيض الدافئ، وأفتح برتقالي عند نفس درجة اللون يبقى فوق 4.5:1 هو
 *  `#BD4F00` بـ 4.57:1 — درجة إضاءة واحدة فرق، وماكو متسع. يعني كل ما يُفتّح البرتقالي
 *  تتسع المسافة بينه وبين توأمه على الأرضية الفاتحة، وهذا بالضبط سبب وجود هذا الرمز
 *  منفصلاً من الأساس. */
 ...
 * من الأرضيتين بحجم النص العادي (الأبيض عليه 1.95:1)؛ استخدم Obsidian، تماماً كما مع البرتقالي، حيثما
 * وجب أن يحمل نصاً على الإطلاق.
 */
/* تحرك مع ORANGE حين فُتّح، ليبقى فوقه لا تحته: كان يجلس 6 درجات إضاءة فوق الإبراز،
   والبرتقالي الجديد (L 62) كان راح يتجاوز Ember القديم (L 56) فتنقلب العائلة ويصيرا
   لونين متقاربين لدرجة أنهما لون واحد. */
export const EMBER = '#FFA552';
