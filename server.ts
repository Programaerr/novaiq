...
//
// كان يشغّل ثلاثة مسارات إدارية (سرد الحسابات، تعطيلها، حذفها). السرد ينتقل الآن إلى قراءة
// جدول `profiles` مباشرة من المتصفح (lib/adminUsers.ts، وسياسات RLS تقصر السرد على الأدمن)،
// والتعطيل والحذف صارا دالّة Supabase Edge Function مستقلّة (supabase/functions/admin-users) —
// كلاهما يحتاج مفتاح service_role، ومفتاح كهذا لا يجوز أن يعيش في خادم Node عادي (هذا الملف
// لا يُنشر أصلاً)، بل في بيئة دالّة محقونة تلقائياً لا يُكتب فيها سرّ إضافي.
//
// وهذا الخادم لم يكن يُنشر يوماً: الإنتاج ملفات ثابتة يبنيها `npm run build` وحده — يعمل محلياً
// فقط عبر `npm run dev`. إبقاء مسارات ميّتة فيه كان سيجعله يقول إن الميزة غير موجودة.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// ترويسات الأمان — نسخة مطابقة لما يضبطه public/_headers على الاستضافة، مكرّرة هنا لأن هذا
// الخادم هو ما يعمل محلياً وعلى أي استضافة Node مستقبلاً؛ الموقع يجب أن يكون محمياً بنفس
// القدر في الحالتين لا في واحدة فقط. راجع تعليقات public/_headers لسبب كل قيمة (خصوصاً
...
// ملفات ثابتة وإعادة توجيه، وهي لا تُحمى بعدّاد في الذاكرة. أمّا الموقع المنشور فخلف الاستضافة
